...
async def document_parser(req: request):
    ...
    local_file = LocalFile(user_id, kb_id, file, file_id, file_name, local_doc_qa.embeddings)
    local_file.parser_file_to_doc(local_doc_qa.get_ocr_result)
    parser_documents = []
...
async def get_files_statu(req: request):
    ...
    if len(file_ids) == 0:
        ...
        file_infos = local_doc_qa.mysql_client.get_files_info(user_id, kb_id, file_ids)
    data = []
    status_count = {}
    msg_map = {'gray': "正在上传中，请耐心等待",
               'red': "split或embedding失败，请检查文件类型，仅支持[md,txt,pdf,jpg,png,jpeg,docx,xlsx,pptx,eml,csv]",
...
async def get_qa_info(req: request):
    ...
    default_need_info = ["qa_id", "user_id", "kb_ids", "query", "model", "product_source", "time_record",
                         "history", "condense_question", "result", "retrieval_documents", "source_documents", "timestamp"]
